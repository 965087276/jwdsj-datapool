...
public interface StatsTableMapper {
    ...
    @Select("select IFNULL(sum(total_records), 0) from stat_table where database_id = #{databaseId}")
    long countDatabaseRecords(long databaseId);

    @Select("select IFNULL(count(*), 0) from stat_table where database_id = #{databaseId}")
